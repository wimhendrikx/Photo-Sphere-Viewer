...
class PhotoSphereViewer {
   ...
   * If the "options" is not defined, the camera will not move and the ongoing animation will continue<br>
   ...
   * @param {PhotoSphereViewer.AnimateOptions} [options] - target position and/or zoom level
   * @param {boolean} [transition=false]
   * @returns {Promise}
   * @throws {PSVError} when another panorama is already loading
   */
  setPanorama(path, options = {}, transition = false) {
    if (this.prop.loadingPromise !== null) {
      return Promise.reject(new PSVError('Loading already in progress'));
    }

    if (typeof options === 'boolean') {
      transition = options; // eslint-disable-line no-param-reassign
