...
import { clone, getClosest, getEventKey, normalizeWheel, throttle } from '../utils';
...
class PSVEventsHandler extends AbstractService {
   ...
   */
  __fullscreenToggled() {
    const enabled = this.psv.isFullscreenEnabled();
     ...
     */
    this.psv.trigger(EVENTS.FULLSCREEN_UPDATED, enabled);
  }
  ...
  __startMoveZoom(evt) {
    ...
    this.state.pinchdist = Math.sqrt(Math.pow(t[0].x - t[1].x, 2) + Math.pow(t[0].y - t[1].y, 2));
    this.state.mouseX = (t[0].x + t[1].x) / 2;
    this.state.mouseY = (t[0].y + t[1].y) / 2;
    this.state.startMouseX = this.state.mouseX;
    this.state.startMouseY = this.state.mouseY;
    this.state.moving = true;
  ...
  __moveZoom(evt) {
    if (this.state.zooming && this.state.moving) {
      ...
      const p = Math.sqrt(Math.pow(t[0].x - t[1].x, 2) + Math.pow(t[0].y - t[1].y, 2));
